                                ...
                                <div class="ng-prslist-img" @click="jump('../jingdian/detail.jsp?id='+item.id)">
                                    <a href="#" class="ng-prsliik">
                                        <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img :src="item.jingdianPhoto?item.jingdianPhoto.split(',')[0]:''" />
                                        </span>
...
<script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery'], function() {
        ...
        function pageList() {
            ...
            // 当前表
         //获取ID为jingdianname的值，值不为空添加param对象中
            ...
            if (jingdianDengjiTypes != null && jingdianDengjiTypes != "" && jingdianDengjiTypes != "null") {
                param['jingdianDengjiTypes'] = jingdianDengjiTypes;
                            ...
                            // 获取列表数据 发送请求
            //成功响应时，将返回的数据列表赋值给Vue实例的dataList属性。
            http.request('jingdian/list', 'get', param, function(res) {
                vue.dataList = res.data.list
                // 分页使用laypage.render方法初始化分页组件
                laypage.render({
                    elem: 'pager',
...
// 这两个函数分别用于获取景点类型和等级
// 成功响应后存储在Vue实例的jingdianTypesList和jingdianDengjiTypesList属性中。
       // 景点类型的查询
